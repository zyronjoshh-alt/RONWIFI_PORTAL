...
(function() {
  ...
  var state = {
    ...
    coinTimerId: null,
    coinTimeoutAt: 0,
    windowSeconds: 60   // Default 60s, will be updated from ESP config
  };
...
/* ---------- INSERT COIN FLOW ---------- */
  function onInsertCoin() {
    ...
    // ✅ FIX: Skip backend if not configured (saves 5s delay in v0)
    var hasBackend = SETTINGS.backend && SETTINGS.backend.base_url;
    ...
    if (hasBackend) {
      ...
      tryBackendCoinStart(function(err, backendResult) {
        ...
        // Backend failed — try ESP
        attemptEspCoinStart();
      });
    } else {
      // No backend configured, go straight to ESP
      attemptEspCoinStart();
    }
  }
  
  function attemptEspCoinStart() {
  ...
  function enterCoinSession(data) {
    // ✅ Use window_seconds from ESP if provided, otherwise default to 60
    if (data.window_seconds) {
      state.windowSeconds = data.window_seconds;
    }
  ...
  function startCoinTimer() {
    var timeoutMs = state.windowSeconds * 1000;
    state.coinTimeoutAt = Date.now() + timeoutMs;
    updateCoinTimerUI();
  ...
  function updateCoinTimerUI() {
    ...
    $('coinTimerFill').style.width = ((remaining / timeoutMs) * 100) + '%';
  }
  ...
  function finalizeCoinSession(reason) {
    ...
    // ✅ FIX: Define vendoIp at the top so it's available for cancel
    ...
    if (reason === 'cancel') {
      ...
      httpPost(cancelUrl, { session_id: state.session ? state.session.session_id : null }, 3000, function(err, resp) {
        if (err || !resp || !resp.ok) {
          console.log('[cancel] ESP cancel request failed, timeout will handle cleanup');
        }
      });
      state.session = null;
    ...
    httpPost(url, {
      ...
      // Redirect to alogin.html with credentials for MikroTik authentication
      if (username && password) {
        setTimeout(function() {
          window.location.href = '/alogin.html?username=' + encodeURIComponent(username) + 
            '&password=' + encodeURIComponent(password);
        }, 1500);
      } else {
        // Fallback: no credentials returned, just refresh
        toast('Warning: No login credentials received', 'error');
        setTimeout(function() {
          refreshSession();
        }, 2000);
      }
